...
<meta charset="UTF-8">
<title>
Profil <?= ucfirst($role) ?> | <?= htmlspecialchars(getSetting('system_name', 'RMBS')) ?>
</title>
...
<h2 style="text-align:center;margin-bottom:10px">
    Profil <?= $role === 'superadmin' ? 'Super Administrator' : 'Administrator' ?>
</h2>
...
/* NAVBAR SCROLL */
let last=0,nav=document.querySelector('.navbar');
window.addEventListener('scroll',()=>{
    let c=window.pageYOffset;
    nav.classList.toggle('hide',c>last&&c>120);
    nav.classList.toggle('shrink',c>90);
    last=c<=0?0:c;
});
